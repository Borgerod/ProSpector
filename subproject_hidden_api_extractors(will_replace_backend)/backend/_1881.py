...
class _1881Extractor:
	...
	def setUrl(self)-> None:
		self.url = self.base_url + self.industry

	def getSoup(self):
		response = requests.request("GET", self.url, headers = self.header)
		return BeautifulSoup(response.content, "html.parser")
	
	def getProfile(self, url:str) -> BeautifulSoup:
		response = requests.request("GET", url, headers = self.header)
		return BeautifulSoup(response.content, "html.parser")
		   
	def parseProfile(self, profile:BeautifulSoup) -> BeautifulSoup:
		return profile.find("ul", class_ = "list-inline-comma") or profile.find("ul", class_ = "box ad-category-text")

	def extractPage(self, soup):
		try:
		...
		'''
		get profiles from link_list (page 1)
		'''
		...
		for url in link_list:
			...
			if self.parseProfile(profile):
				print("True")
				company_name = profile.find("hl", class_ = "details-name")
				org_num = self.getOrgnum()
	...
	def getOrgnum(self):
		...
		try:
			return int(soup.find("th", string = " Org nr ").find_next().text.replace('\xa0', ''))
		except AttributeError:
			print(url, self.profile_url)

	def worker(self, industry):
		self.industry = industry
		self.url = self.base_url + self.industry
		soup = self.getSoup()
		self.extractPage(soup)
		page_num = 1
	...
	def runExtraction(self) -> None:
		industries = getAll1881Industries()[:1]
		with Pool() as pool:
