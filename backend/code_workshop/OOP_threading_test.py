...
class Extractor():	
	...
	def getData(self, input_array: np.ndarray,) -> np.ndarray or str:
		...
		driver = prepDriver(self.search_term)
		...
		if checkGoogleAlarmTrigger(driver):
			...
			# print(self.org_num, self.search_term, self.is_reqistered, self.is_claimed, self.has_info, False)
			# array = np.array((self.org_num, self.search_term, self.is_reqistered, self.is_claimed, self.has_info, False), dtype = object)
			# return array
			# return np.array((self.org_num, self.search_term, self.is_reqistered, self.is_claimed, self.has_info, False), dtype = object) 
			# return [self.org_num, self.search_term, self.is_reqistered, self.is_claimed, self.has_info, False]
			# getResult(self.org_num, self.search_term, self.is_reqistered, self.is_claimed, self.has_info, False)
			return self.org_num, self.search_term, self.is_reqistered, self.is_claimed, self.has_info, False
	...
	# 	base_url = "https://www.google.com/search?q=" 
	# 	driver = getDriver()
	# 	driver.get(linkBuilder(base_url, self.search_term))
	# 	driver.current_window_handle  #* CATCHPA SOLVER
	# 	time.sleep(getShortBreak())
	# 	self.driver = driver
...
def googleExtractor(**kwargs: str):
	...
	for chunk in nested_input_array:
		for i in chunk:
			# result = Extractor(i)
			twrv = ThreadWithReturnValue(target=Extractor.getData, args=(chunk))
			twrv.start()
			print(twrv.join())
	...
	# 					return_value = thread.join()
						# print(result)
						# results = [Extractor(i) for i in chunk]
		# 				result_list = []
		# 				error_count = []
		# 				pbar3.update(1)  
