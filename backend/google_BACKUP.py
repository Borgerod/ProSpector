...
from SQL.postgres import databaseManager
	...
	with tqdm(total = len(nested_input_array)) as pbar1: 
		with tqdm(total = len(input_array)) as pbar2:
			...
			with ThreadPoolExecutor(max_workers=min(32, (os.cpu_count() or 1) + 4)) as executor: 
				for chunk in nested_input_array:
					with tqdm(total = len(chunk)) as pbar3:
						futures = list(tqdm(executor.map(Extration().getData, chunk), total=len(chunk)))
						result_list = []
						error_count = []
						for future in futures:
							(error_count.append(future) if None in future else result_list.append(future))	
							# print(future)
						pbar1.update(1)
						pbar2.update(chunksize) 	


						# futures = [executor.map(Extractor, chunk)]
						# futures = list(tqdm(executor.map(E.getData, chunk), total=len(chunk)))
						# futures = 
