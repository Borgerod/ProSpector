...
# def findPlace(api_key, name):
	# '''
	# Builds url string for google api engine
	# '''
	# # Museum%20of%20Contemporary%20Art%20Australia
...
def formatParameters():
	...
	formating = '%2C'
	formated_params = ['%2C'+i for i in params]
	formated_str = first_param+''.join(formated_params)
	return formated_str


def placeDetails(api_key, place_id):
	parameters = formatParameters()
	url = f'''https://maps.googleapis.com/maps/api/place/details/json?place_id={place_id}&fields={parameters}&key={api_key}'''
	# print(url)
	payload = {}
	headers = get_hdr()
	response = requests.request("GET", url, headers = headers, data=payload)	
...
def textSearch(api_key, text_input):
	'''text_input = address eller address'''

	url = f'''https://maps.googleapis.com/maps/api/place/textsearch/json?query={text_input}&key={api_key}'''
	# print(url)
	payload = {}
	headers = get_hdr()
	response = requests.request("GET", url, headers = headers)	
...
def main():
	...
	'''Felleskatalogen AS,Oslo,23 16 15 50,['Adresseringsleverandører'],965614923'''
	search_inputs = ['Felleskatalogen AS']
	name = 			['Felleskatalogen AS']
	for input_ in search_inputs:
		place_id = textSearch(api_key, input_)
		output = placeDetails(api_key, place_id)
