...
# from postgres import cleanUp, fetchData, postLastUpdate, replacetData, insertData #! SOON TO BE REPLACED 
...
def datasetEditor(df) -> pd.DataFrame:
	'''
	...
	# return df[df.columns.intersection(keep_list)]
	...
	df['postadresse'] = df['postadresse'].apply(json.dumps)
	df['forretningsadresse'] = df['forretningsadresse'].apply(json.dumps)
	df = brregCleanUp(df) # Removes all rows which is bankrupt etc.   
	return df
